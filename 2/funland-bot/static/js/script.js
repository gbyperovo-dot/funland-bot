...
document.addEventListener('DOMContentLoaded', function() {
    initializeChat();
    loadDisplayMenu(); // 🆕 Загружаем меню с отображаемым текстом
    setupEventListeners();
    setupCalculator();
});
...
async function loadDisplayMenu() {
    try {
        ...
        menuItems.forEach(item => {
            ...
            button.onclick = () => sendMessage(item.question);
...
// Обработчик для обновления меню при изменениях (для админки)
if (typeof window.updateMenu === 'undefined') {
